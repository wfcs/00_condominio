...
const Login: React.FC<LoginProps> = ({ onLogin, users }) => {
  ...
  const handleRecoverPassword = async (e: React.MouseEvent) => {
    ...
    if (!email || email.trim() === '') {
      setError('Por favor, digite seu e-mail antes de recuperar a senha.');
      return;
    }
    ...
    <div className="min-h-screen w-full flex items-center justify-center p-4 relative overflow-hidden bg-gradient-to-br from-[#48cae4] via-[#caf0f8] to-[#48cae4] animate-gradient-slow">
      ...
      <div className="bg-white/80 dark:bg-slate-900/90 backdrop-blur-2xl rounded-[3rem] shadow-[0_32px_64px_-12px_rgba(0,0,0,0.15)] p-8 md:p-14 max-w-md w-full relative z-10 border border-white/40">
        ...
        {recoveryStatus?.loading && (
          <div className="mb-8 bg-blue-50 border-l-4 border-brand-3 text-brand-1 p-4 rounded-2xl text-xs font-bold flex items-center gap-3 animate-pulse">
            <i className="fa-solid fa-spinner fa-spin text-lg"></i>
            Gerando senha temporária...
          </div>
        )}

        {recoveryStatus?.success && recoveryStatus.tempPass && (
          <div className="mb-8 bg-emerald-50 border-l-4 border-emerald-500 text-emerald-700 p-5 rounded-2xl animate-in fade-in slide-in-from-top-2">
            <div className="flex items-start gap-3">
              <i className="fa-solid fa-circle-check text-xl mt-0.5"></i>
              <div className="flex-1">
                <p className="text-xs font-black mb-2">✅ Senha temporária gerada com sucesso!</p>
                <div className="bg-white/60 border-2 border-dashed border-emerald-300 rounded-xl p-3 mt-3">
                  <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1">Sua senha temporária:</p>
                  <p className="text-2xl font-black text-brand-1 tracking-wider font-mono">{recoveryStatus.tempPass}</p>
                </div>
                <p className="text-[10px] text-emerald-600 mt-3 font-semibold">
                  💡 Use esta senha para fazer login. Você poderá alterá-la depois.
                </p>
              </div>
            </div>
          </div>
